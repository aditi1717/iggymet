...
export default function DeliveryPrivacyPolicy() {
  ...
  const fetchPrivacyData = async () => {
    try {
      setLoading(true)
      const response = await api.get(API_ENDPOINTS.ADMIN.DELIVERY_PRIVACY, { contextModule: "admin" })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(
        API_ENDPOINTS.ADMIN.DELIVERY_PRIVACY,
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50 p-4 lg:p-6">
      <div className="max-w-6xl mx-auto">
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900">Delivery Boy Privacy Policy</h1>
          <p className="text-sm text-slate-600 mt-1">Manage your Delivery Partner Privacy Policy content</p>
        </div>
