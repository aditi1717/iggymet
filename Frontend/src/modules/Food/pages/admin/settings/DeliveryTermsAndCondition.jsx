...
export default function DeliveryTermsAndCondition() {
  ...
  const [termsData, setTermsData] = useState({
    title: 'Delivery Boy Terms and Conditions',
    content: ''
  })
  ...
  const fetchTermsData = async () => {
    try {
      setLoading(true)
      const response = await api.get(API_ENDPOINTS.ADMIN.DELIVERY_TERMS, { contextModule: "admin" })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(
        API_ENDPOINTS.ADMIN.DELIVERY_TERMS,
      ...
      toast.error(error.response?.data?.message || 'Failed to save delivery terms and conditions')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50 p-4 lg:p-6">
      <div className="max-w-6xl mx-auto">
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900">Delivery Boy Terms And Conditions</h1>
          <p className="text-sm text-slate-600 mt-1">Manage your Delivery Partner Terms and Conditions content</p>
        </div>
        ...
        <div className="flex justify-end mt-6">
          <button
            ...
            disabled={saving}
            className="px-6 py-2.5 bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
